...
| # | 기능 | DAB 리소스 타입 | 배포 결과 |
|---|------|---------------|----------|
| 1 | **Notebook** | `jobs` (notebook_task) | 실행 성공 |
| 2 | **Job (Workflow)** | `jobs` | 실행 성공 |
| 3 | **SDP Pipeline (Lakeflow)** | `pipelines` | 실행 성공 |
| 4 | **Lakebase** | `postgres_projects` + `postgres_branches` | 자동 프로비저닝 |
| 5 | **Databricks App** | `apps` | App 실행 중 |
| 6 | **AI/BI Dashboard** | `dashboards` | 즉시 조회 가능 |
| 7 | **ML Custom Model** | `experiments` + `registered_models` | 실행 성공 |
| 8 | **Model Serving** | `model_serving_endpoints` | Endpoint 활성 |
...
| # | 기능 | 배포 방법 | 배포 결과 |
|---|------|----------|----------|
| 9 | **Unity Catalog Function** | SQL notebook | 실행 성공 |
| 10 | **Metric View** | SQL notebook (WITH METRICS) | 실행 성공 |
| 11 | **Genie** | Python notebook (REST API) | 실행 성공 |
| 12 | **AgentBricks Knowledge Assistant** | Python notebook (SDK) | 실행 성공 |
...
| 항목 | 값 |
|------|-----|
| 워크스페이스 | (your-workspace.cloud.databricks.com) |
