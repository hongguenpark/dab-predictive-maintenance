...
-- 1. 장비 가동률 메트릭
CREATE OR REPLACE VIEW hg_demos.predictive_maintenance.mv_equipment_availability AS
SELECT
  equipment_id,
...
FROM hg_demos.predictive_maintenance.sensor_readings
GROUP BY equipment_id, DATE(timestamp);

-- COMMAND ----------

-- 2. 평균 고장 간격 (MTBF) 메트릭
CREATE OR REPLACE VIEW hg_demos.predictive_maintenance.mv_mtbf AS
SELECT
  equipment_id,
...
CREATE OR REPLACE VIEW hg_demos.predictive_maintenance.mv_oee AS
SELECT
  e.equipment_id,
  e.equipment_name,
  e.criticality,
...
JOIN (
  ...
  FROM hg_demos.predictive_maintenance.mv_equipment_availability
  GROUP BY equipment_id
...
-- 검증 쿼리
SELECT * FROM hg_demos.predictive_maintenance.mv_oee ORDER BY oee_pct ASC;
